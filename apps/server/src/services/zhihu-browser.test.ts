...
function createFakePage(options?: { loginPage?: boolean }) {
  ...
  return {
    ...
    locator(selector: string) {
      if (selector.includes("h1[contenteditable")) return title;
      if (selector.includes("标题") || selector.includes("titleInput")) return title;
      if (selector.includes("DraftEditor") || selector.includes("内容") || selector.includes("正文") || selector.includes("contenteditable")) return body;
...
describe("zhihu browser publisher", () => {
  ...
  it("supports a Zhihu editor title that is only exposed as contenteditable", async () => {
    ...
    const page = {
      ...
      locator(selector: string) {
        if (selector.includes("h1[contenteditable") || selector.includes('[role="textbox"]') || selector.includes("article-title")) return title;
        if (selector.includes("正文") || selector.includes("contenteditable") || selector.includes("DraftEditor")) return body;
        if (selector.includes("确认")) return confirm;
        return hidden;
      },
      getByPlaceholder() {
